...
export function getRateLimiter(config: RuntimeConfig['fhir']): RateLimitRequestHandler {
  if (!handler) {
...
function getRateLimitForRequest(config?: RuntimeConfig['fhir']): (req: Request) => Promise<number> {
